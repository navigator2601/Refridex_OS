...
"""
Обробка натискань кнопок головного меню, навігації та команд.
"""
...
from handlers.catalog_handler import show_catalog_handler_message
...
# Обробник для ручного введення "каталог"
@router.message(F.text.lower() == "каталог")
async def show_catalog_from_text_handler(message: types.Message, db_pool: asyncpg.Pool):
    """
    Обробляє запит на показ каталогу, введений вручну.
    """
    await show_catalog_handler_message(message, db_pool)
...
# Обробляє натискання кнопки "Пошук"
...
# Обробляє текстовий запит у стані пошуку
...
async def handle_search_query(message: types.Message, db_pool: asyncpg.Pool, state: FSMContext) -> None:
    ...
    if user_query.lower() in ("скасувати", "/cancel"):
        await state.set_state(MenuStates.main_menu)
        access_level = await get_user_access_level(db_pool, user_id)
    ...
    if search_results:
        ...
        await message.answer(
            f"<b>Знайдено {len(search_results)} результатів:</b>\n\n"
            f"{formatted_results}",
